...
$$
\min_{u_0, u_1, \ldots, u_{N_c-1}} J = \sum_{k=1}^{N_p} w_{track} (T_s(k) - T_{ref})^2 + w_{terminal} (T_s(N_p) - T_{ref})^2 + w_{rate} (u_0 - u_{last})^2 + \sum_{k=1}^{N_c-1} w_{rate} (u_k - u_{k-1})^2
$$
...
| $w_{track}$ | 10.0 | 跟踪权重，越大跟踪越紧 |
| $w_{terminal}$ | 50.0 | 终端权重，确保终端状态接近目标 |
| $w_{rate}$ | 0.1 | 变化率权重，越大功率变化越平滑 |
...
$$
J_{track} = w_{track} \sum_{k=1}^{N_p} (T_s(k) - T_{ref})^2
$$

### A.2 终端状态惩罚项

$$
J_{terminal} = w_{terminal} (T_s(N_p) - T_{ref})^2
$$

### A.3 变化率惩罚项

$$
J_{rate} = w_{rate} (u_0 - u_{last})^2 + w_{rate} \sum_{k=1}^{N_c-1} (u_k - u_{k-1})^2
$$

### A.4 总目标函数

$$
J = J_{track} + J_{terminal} + J_{rate}
$$
