...
class LoggingControlWrapper:
    ...
    def __init__(self, heater, wrapped_control=None):
        self.wrapped_control = wrapped_control or ControlBangBang(
            {"max_delta": 2.0}, heater
        )
        self.heater = heater
        # 日志现在包含时间、温度和PWM值
        self.log = []
    ...
    def temperature_update(self, read_time, temp, target_temp):
        if self.logging:
            # 在温度更新时记录时间、温度和目标温度
            pwm_value = getattr(
                self.heater, "last_pwm_value", 0.0
            )  # 尝试获取最后的PWM值
        ...
        if hasattr(self.wrapped_control, "temperature_update"):
            return self.wrapped_control.temperature_update(
                read_time, temp, target_temp
            )
        else:
    ...
    def check_busy(self, eventtime, smoothed_temp, target_temp):
        if hasattr(self.wrapped_control, "check_busy"):
            return self.wrapped_control.check_busy(
...
class TempStepTest(MpcCalibrate):
    def __init__(self, config):
        ...
        heater_name = config.get_name().split()[
            -1
        ]  # 获取配置名称的最后一部分，例如"extruder"
    ...
    cmd_TEMP_STEP_TEST_help = (
        "Run temperature step test with different controllers"
    )
    ...
    def run(self, gcmd):
        ...
        controller_type = gcmd.get(
            "CONTROLLER", "neno"
        ).lower()  # 默认为neno（继电器控制器）
        threshold_temp = gcmd.get_float(
            "THRESHOLD", max(50.0, min(100, target_temp - 100.0))
        )

        gcmd.respond_info(
            f"Starting step test with {controller_type} controller, target temp {target_temp}°C for {duration}s"
        )
        ...
        if controller_type == "pid":
            ...
            # 使用BangBang控制器
            control = LoggingControlWrapper(
                self.heater, ControlBangBang({"max_delta": 2.0}, self.heater)
            )
        else:  # 默认为neno（继电器控制器），即TuningControl
            control = LoggingControlWrapper(
                self.heater, TuningControl(self.heater)
            )
        ...
        try:
            ...
            gcmd.respond_info(
                f"Step test with {controller_type} controller completed. Collected {len(log)} samples."
            )
    ...
    def step_test(self, gcmd, target_temp, control, duration=60):
        ...
        """
        gcmd.respond_info(
            f"Starting step test with {control.get_type()} controller, "
            f"target temp {target_temp}°C"
        )

        # 首先加热到25度并等待稳定
        gcmd.respond_info(
            "Pre-heating to 25°C and waiting for stabilization..."
        )
        ...
        # 设置目标温度并开始阶跃测试
        gcmd.respond_info(
            f"Starting step test to target temperature: {target_temp}°C"
        )
        ...
        def ramp_check(eventtime):
            ...
            gcmd.respond_info(
                f"Ramping up - Time: {(eventtime - start_time):.1f}s, Temperature: {temp:.2f}°C",
                log=False,
            )

            # 检查是否达到目标温度
            if (
                not temp_reached[0] and temp >= target_temp * 0.98
            ):  # 达到目标温度的98%
        ...
        if reached_target_time[0] is None:
            ...
            gcmd.respond_info(
                "Did not reach target temperature within duration, continuing sampling..."
            )

        # 继续采样60秒
        gcmd.respond_info(
            "Continuing sampling for 60 seconds after reaching target..."
        )
        ...
        def sampling_check(eventtime):
            temp, _ = self.heater.get_temp(eventtime)
            gcmd.respond_info(
                f"Post-target sampling - Time: {(eventtime - start_time):.1f}s, Temperature: {temp:.2f}°C",
                log=False,
            )
        ...
        gcmd.respond_info(
            "Cooling down to room temperature (~30°C) after stabilization..."
        )
        ...
        def cooling_check(eventtime):
            temp, _ = self.heater.get_temp(eventtime)
            gcmd.respond_info(
                f"Cooling down - Temperature: {temp:.2f}°C", log=False
            )
            # 冷却直到温度降到30度或最长3分钟
    ...
    def save_heatup_data(self, gcmd, samples):
        ...
        try:
            ...
            with open(file_path, "w") as f:
                ...
                if samples:
                    ...
                    for t, temp, pwm, target in samples:
                        ...
                        f.write(
                            f"{rel_time:.3f},{temp:.2f},{pwm:.4f},{target:.2f}\n"
                        )
                else:
            ...
            gcmd.respond_info(
                f"Heatup test data (with PWM) saved to {file_path}"
            )
