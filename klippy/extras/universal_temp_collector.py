...
class UniversalTempCollector:
    ...
    def __init__(self, config):
        ...
        logging.info(
            ...
            self.data_dir, self.default_sample_rate
        )
        ...
        if sources_str:
            for source_def in sources_str.split(','):
                ...
                if ':' in source_def and source_def:
                    ...
                    logging.info("  添加内联数据源: %s (%s)", src_name, src_type)
        ...
        for src_config in self.source_configs:
            ...
            try:
                ...
                if adapter:
                    ...
                    logging.info("成功注册数据源 '%s' (类型: %s)", name, src_type)
    ...
    def remove_source(self, name):
        ...
        if name in self.sources:
            ...
            # 同时从配置列表中移除
            self.source_configs = [
                c for c in self.source_configs 
                if c.get('name') != name
            ]
            ...
            try:
                ...
                if temp is not None:
                    sample = {
                        ...
                        'temperature': temp,
                        'target_temperature': target if target is not None else '',
                        **extra
                    }
    ...
    def _start_collection(self, experiment_name=None):
        ...
        logging.info(
            ...
            active_count, self.current_sample_rate
        )
        return True
            ...
            with open(filepath, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                for sample in self.data_buffer:
                    ...
                    if 'timestamp' in row and start_time:
                        row['relative_time'] = round(row['timestamp'] - start_time, 6)
    ...
    def cmd_TEMP_MULTI_COLLECT(self, gcmd):
        ...
        # 应用过滤（如果有指定）
        if sources_filter:
            filter_list = [s.strip() for s in sources_filter.split(',') if s.strip()]
            for name, adapter in self.sources.items():
        ...
        if self._start_collection(experiment_name):
            active_sources = [
                n for n, a in self.sources.items() if a.enabled
            ]
            gcmd.respond_info(
                f"多对象温度数据采集已启动:\n"
    ...
    def cmd_TEMP_MULTI_STOP(self, gcmd):
        ...
        if success:
            gcmd.respond_info(f"数据已保存: {result}\n总样本数: {len(self.data_buffer)}")
    ...
    def _show_data_preview(self, gcmd):
        ...
        for sample in self.data_buffer:
            ...
            if name not in stats:
                stats[name] = {'count': 0, 'temps': [], 'min': float('inf'), 'max': float('-inf')}
        ...
        for name, stat in sorted(stats.items()):
            avg = sum(stat['temps']) / len(stat['temps']) if stat['temps'] else 0
    ...
    def cmd_TEMP_MULTI_STATUS(self, gcmd):
        ...
        status_text.append(f"  采集状态: {'采集中' if self.is_collecting else '空闲'}")
        ...
        if show_values:
            ...
            for name, adapter in sorted(self.sources.items()):
                try:
                    ...
                    target_str = f", 目标: {target:.1f}" if target is not None else ""
    ...
    def cmd_TEMP_MULTI_ADD_SOURCE(self, gcmd):
        ...
        if src_type not in valid_types:
            raise gcmd.error(f"无效的类型 '{src_type}'，有效类型: {valid_types}")
